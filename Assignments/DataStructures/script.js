...
// const [{ title, author, ISBN }] = books; // My solution
...
// const printBookInfo = function (b) {
//   const { title, author, year = 'year unknown' } = books[b];
...
for (let i = 0; i < books.length; i++) {
  // books[i].edition ??= 1; my solution
  books[i].edition ||= 1;
  // console.log(i, books[i].edition);
}
...
// 11.1 Below is the entries variable that stores an empty array. Use the for-of loop together with the Object.keys() method to loop over the thirdParty.goodreads property (array) of the first book object from the books array. For each key, push a new array that contains that key to the entries array. In the end, the entries array should be filled with arrays containing keys: [['rating'], ['ratingsCount'], ['reviewsCount'], ['fiveStartRatingCount'], ['oneStartRatingCount']]
